...
import (
	...
	"github.com/vmware-tanzu/vm-operator-api/api/v1alpha2/cloudinit"
	"github.com/vmware-tanzu/vm-operator-api/api/v1alpha2/sysprep"
)
...
type VirtualMachineBootstrapSpec struct {
	...
	// CloudInit may be used to bootstrap Linux VMs with Cloud-Init or
	// Windows VMs with Cloudbase-Init.
	//
	// The guest's networking stack is configured by Cloud-Init (Linux) /
	...
	// LinuxPrep may be used to bootstrap Linux VMs.
	//
	...
	// Sysprep may be used to bootstrap Windows VMs.
	//
	// The guest's networking stack is configured by Guest OS Customization
	// (GOSC).
	//
	...
	// VAppConfig may be used to bootstrap VMs that rely on vApp properties
	// (how VMware surfaces OVF properties on VMs) to transport data into the
	// guest.
	//
	// The guest's networking stack may be configured using either vApp
	// properties or Guest OS Customization (GOSC).
	//
	// Many OVFs define one or more properties that are used by the guest to
	// bootstrap its network stack. If the VirtualMachineImage defines one or
	...
	// configuring either the LinuxPrep or Systeprep bootstrap providers.
	//
...
type VirtualMachineBootstrapCloudInitSpec struct {
	...
	// bootstrap the VM.
	//
	// Please note while this field does not expose every CloudConfig module,
	// it is possible to specify a complete CloudConfig via the RawCloudConfig
	// field, and the information from this field will be merged with the
	// raw CloudConfig prior to bootstrapping the VM.
	//
	// +optional
	...
	//
	// Please note the CloudConfig and RawCloudConfig fields will be merged
	// prior to bootstrapping the VM.
	//
	// +optional
...
type VirtualMachineBootstrapSysprepSpec struct {
	...
	//
	// +optional
	RawSysrep corev1.SecretKeySelector `json:"rawSysprep,omitempty"`
}
